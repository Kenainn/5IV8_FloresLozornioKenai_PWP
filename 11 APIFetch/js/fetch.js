...
const pokedex = ()=>{
    ...
    //tiene que cambiar los elementos de la imagen, para ello tenemos que crear un template que se encarge de encadenar los datos
    const imageTemplate = "<img clas='pokedipslay' scr='{imgSrc}' alt='pokedisplay'/>"

    //necesitamos un objeto que se encarge de guardar las rutas de las imagenes que vamos a cambiar dependiendo de si es una busqueda, si lo encontro o no a el pokemon
    const images = {
    ...
    //necesitamos un objeto tipo array que guarde los botones con tipo de referencia
    ...
    const processPokemonType = (pokemonData) =>{
        //primero necesitamos obtener el tipo de pokemon y la clase para que se modifique en el html, ya que tenemos eso tendremos que obtener stats,moves,abilities
    
        let pokemonType = "";
        //utilizo una busqueda de la clase de pokemon, eso se refiere a l tipo de pokemon
        ...
        if(currentClassType){
            containers.pokemonTypesContainer.classList.remove(currentClassType);
            containers.pokemonAbilitiesElement.classList.remove(currentClassType);
        }//ahora tengo que agregar lo nuevo
        containers.pokemonTypesContainer.classList.add(firstClass);
        containers.pokemonAbilitiesElement.classList.add(firstClass);
        //debo de agregar las etiquetas creadis dentro  del forEach
    ...
    const processPokemonStats = (pokemonData) =>{
        pokemonData.stats?.foreach((pokemonstatData) =>{
            //vamos a evaluar si encuentra el nombre de las estadisticas para colocarlo en su contenedor correspondiente
            switch(pokemonstatData.stat.name){
                case 'hp':
                    pokemonStatsElements.hp.innerHTML = pokemonDatastatData.base_stat;
                    ...
                    break;
                case 'attack':
                    pokemonStatsElements.attack.innerHTML = pokemonDatastatData.base_stat;
                    ...
                    break;
                case 'defense':
                    pokemonStatsElements.defense.innerHTML = pokemonDatastatData.base_stat;
                    ...
                    break;
                case 'special-attack':
                    pokemonStatsElements.specialAttack.innerHTML = pokemonDatastatData.base_stat;
                ...
                case 'special-defense':
                    pokemonStatsElements.specialDefense.innerHTML = pokemonDatastatData.base_stat;
                    ...
                    break;
                case 'speed':
                    pokemonStatsElements.speed.innerHTML = pokemonDatastatData.base_stat;
